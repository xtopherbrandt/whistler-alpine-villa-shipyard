...
function isOverlapError(e: unknown): boolean {
  return e instanceof Error && (e as NodeJS.ErrnoException).code === 'OVERLAP'
}
      ...
      async (tx) => {
        const conflict = await tx.stay.findFirst({
          where: {
            unitId,
            status: 'CONFIRMED',
            checkInDate: { lt: toUtcDate(input.checkOutDate) },
            checkOutDate: { gt: toUtcDate(input.checkInDate) },
          },
        })
        ...
        return tx.stay.create({
          data: {
            ...
            checkInDate: toUtcDate(input.checkInDate),
            checkOutDate: toUtcDate(input.checkOutDate),
...
export async function editStay(stayId: string, input: StayInput): Promise<ActionResult<Stay>> {
  ...
  if (existing.status !== 'CONFIRMED') return { data: null, error: 'Forbidden' }
  if (Number(input.unitId) !== existing.unitId) return { data: null, error: 'Unit cannot be changed after registration' }
