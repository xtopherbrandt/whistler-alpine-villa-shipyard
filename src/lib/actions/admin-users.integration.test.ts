...
describe('updateUserUnits — auto-cancels future stays', () => {
  ...
  it('auto-cancels stays checking in today (boundary) when a unit is removed', async () => {
    const { updateUserUnits } = await import('@/lib/actions/admin-users')

    const unitId = 9903
    await db.unit.upsert({
      where: { id: unitId },
      create: { id: unitId, description: 'Integration test unit today-boundary' },
      update: {},
    })
    createdUnitIds.push(unitId)
    ...
    const shareholderUser = await db.user.create({
      data: {
        name: 'T044 Today Boundary Shareholder',
        email: `t044-today-${Date.now()}@integration-test.invalid`,
        isShareholder: true,
        isActive: true,
      },
    })
    ...
    await db.userUnit.create({ data: { userId: shareholderUser.id, unitId } })

    const todayUtcMidnight = new Date(new Date().toISOString().slice(0, 10) + 'T00:00:00.000Z')
    const checkOut = new Date(todayUtcMidnight)
    checkOut.setDate(checkOut.getDate() + 3)
